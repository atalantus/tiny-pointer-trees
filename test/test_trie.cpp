...
TEST(TestTrie, TinyTrie2RemoveDeletesEmptyNodes) {
  /*
    ...
    *               "bca"
    */
  const std::vector<std::string> testWords{"", "a", "ab", "ac", "b", "bc",
                                           "bca"};
...
TEST(TestTrie, ValueTrie) {
  ValueTrie<uint32_t> trie;
...
TEST(TestTrie, TrieUtilNodeCountsSmall) {
  ...
  for (int i = 0; i < 256; ++i) {
    switch (i) {
      case 0: ASSERT_EQ(node_counts[i], 3);
        break;
      case 1: ASSERT_EQ(node_counts[i], 2);
        break;
      case 2: ASSERT_EQ(node_counts[i], 2);
        break;
      default: ASSERT_EQ(node_counts[i], 0);
        break;
    }
  }
  ...
  ASSERT_EQ(testWords.size(), trie.size());
  ASSERT_EQ(trie.node_count(),
            std::reduce(node_counts.begin(), node_counts.end()) - node_counts[0
            ]);
}
...
TEST(TestTrie, TrieUtilNodeCountsSmallWithDuplicates) {
   ...
   *          "bca" "bcb"
   */
  const std::vector<std::string> testWords{"", "a", "a", "ab", "ac", "ac", "b",
                                           "bca", "bca", "bcb", "c"};
  const auto node_counts = compute_node_counts(testWords);

  for (int i = 0; i < 256; ++i) {
    switch (i) {
      case 0: ASSERT_EQ(node_counts[i], 5);
        break;
      case 1: ASSERT_EQ(node_counts[i], 1);
        break;
      case 2: ASSERT_EQ(node_counts[i], 2);
        break;
      case 3: ASSERT_EQ(node_counts[i], 1);
        break;
      default: ASSERT_EQ(node_counts[i], 0);
        break;
    }
...
TEST(TestTrie, TrieUtilNodeCountsFull) {
  ...
  for (int i = 0; i < 257; ++i) {
    if (node_counts[i] > 0) {
      std::cout << "Amount of nodes with " << i << " children: " << node_counts[
        i] << std::endl;
    }
  }
