...
TEST(TestTinyPtr, TinyPtrParametrization) {
  ...
  auto t2 = DerefTableU16S8::TinyPtrT(
      DerefTableU16S8::ENTRIES_PER_BIN_COUNT - 1, 255, true);
...
void TestDereferenceTable() {
  ...
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    switch (i % 3) {
      case 0:
        *deref_table.dereference(bin1[i].first, {0, 2}) = i;
        break;
      case 1:
        *deref_table.dereference(bin2[i].first, {0, 1}) = i;
        break;
      case 2:
        *deref_table.dereference(bin3[i].first, {2, 0}) = i;
        break;
  ...
  // delete all bin1 pointers
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    deref_table.free(bin1[i].first, {0, 0});
  }

  ASSERT_EQ(deref_table.size(), 2 * ENTRY_COUNT);

  // move all bin2 pointers to bin1
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    bin1[i] = deref_table.move(bin2[i], {1, 1}, {0, 0});
  }
  ...
  // assert values
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    uint32_t val1 = i, val2 = i;

    switch (i % 3) {
      case 0:
        break;
      case 1:
        val1 = *deref_table.dereference(bin1[i].first, {0, 0});
        val2 = *bin1[i].second;
        break;
      case 2:
        val1 = *deref_table.dereference(bin3[i].first, {2, 0});
  ...
  // move pointers back from bin1 into bin2 and allocate new bin1
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    bin2[i] = deref_table.move(bin1[i], {0, 0}, {1, 1});
    bin1[i] = deref_table.allocate({0, 0}, SPECIAL_VALUE);
  ...
  for (int i = 0; i < ENTRY_COUNT; ++i) {
    ...
    switch (i % 3) {
      case 0:
        ...
        break;
      case 1:
        val1 = *deref_table.dereference(bin2[i].first, {1, 1});
        val2 = *bin2[i].second;
        break;
      case 2:
        val1 = *deref_table.dereference(bin3[i].first, {2, 0});
...
void ASSERT_ALL_POINTERS_UNIQUE(const std::vector<uint32_t*>& ptrs) {
  ...
  for (auto* p : ptrs) {
    ASSERT_TRUE(
        seen.insert(p).second) << "duplicate pointer returned from allocate: "
 << p;
  }
}
} // namespace
...
TEST(TestTinyPtrThreadSafety, ConcurrentAllocateSameBucketPair) {
  ...
  const size_t ALLOCS_PER_THREAD =
      (2 * DerefTable<uint32_t>::ENTRIES_PER_BIN_COUNT) / THREADS;
...
TEST(TestTinyPtrThreadSafety, ConcurrentAllocateDisjointBuckets) {
  ...
  constexpr size_t ALLOCS_PER_THREAD = DerefTable<
    uint32_t>::ENTRIES_PER_BIN_COUNT;
  const unsigned int THREADS = worker_thread_count();
  ...
  run_on_threads(THREADS, [&](unsigned int t) {
    ...
    for (size_t it = 0; it < ITERATIONS; ++it) {
      ...
      if (live[slot].alive) {
        ...
        ASSERT_EQ(
            *table.dereference(live[slot].tp, {live[slot].h1, live[slot].h2}),
...
TEST(TestTinyPtrThreadSafety, ConcurrentFillToCapacity) {
  ...
  // smallish so we actually hit capacity under contention
  const unsigned int THREADS = worker_thread_count();
...
TEST(TestTinyPtrThreadSafety, FillAndDrainCycles) {
  ...
  for (int cycle = 0; cycle < CYCLES; ++cycle) {
    std::vector<std::vector<std::pair<
      tnyptr_t, std::pair<uint64_t, uint64_t> > > > owned(THREADS);
...
TEST(TestTinyPtrThreadSafety, ConcurrentDereferenceWrites) {
  ...
  run_on_threads(THREADS, [&](unsigned int t) {
    for (size_t w = 0; w < WRITES_PER_SLOT; ++w) {
      for (auto& o : owned[t]) {
        *table.dereference(o.tp, {o.h1, o.h2}) = static_cast<uint32_t>(
          (t << 20) | w);
      }
    }
  });
