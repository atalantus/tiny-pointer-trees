...
Tree::Tree(LoadKeyFunction loadKey, size_t n4_size, size_t n16_size,
           size_t n256_size, size_t leaf_size) : deref_tables(n4_size, n16_size,
                                                   n256_size, leaf_size),
                                                 root(N256::Create(
                                                     nullptr, 0, {0, 0},
                                                     deref_tables)),
  ...
  while (true) {
    switch (checkPrefix(node, k, level)) {
      ...
      case CheckPrefixResult::Match:
        ...
        parentNode = node;
        nodeTinyPtr = N::getChild(k[level], parentNode);
    ...
    switch (prefixResult) {
      ...
      case PCEqualsResults::BothMatch: {
        ...
        if (startLevel != endLevel) {
          std::tuple<uint8_t, N*> children[256];
          uint32_t childrenCount = 0;
          v = N::getChildren(node, startLevel, endLevel, children,
                             childrenCount);
          for (uint32_t i = 0; i < childrenCount; ++i) {
            const uint8_t k = std::get<0>(children[i]);
            N* n = std::get<1>(children[i]);
      ...
      case CheckPrefixResult::Match: {
        ...
        if (N::isLeaf(nextNode)) {
          ...
          if (node->getCount() == 2 && parentNode != nullptr) {
            ...
            N* secondNodeN;
            uint8_t secondNodeK;
            std::tie(secondNodeN, secondNodeK) = N::getSecondChild(
                node, nodeKey);
            ...
            N::removeAndUnlock(node, v, k[level], parentNode, parentVersion,
