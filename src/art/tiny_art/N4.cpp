...
void N4::insert(uint8_t key, ArtTinyPtr n) {
  unsigned pos;
...
bool N4::change(uint8_t key, ArtTinyPtr val) {
  for (uint32_t i = 0; i < count; ++i) {
    if (keys[i] == key) {
      children[i] = val;
      return true;
...
ArtTinyPtr N4::getChild(const uint8_t k) const {
  for (uint32_t i = 0; i < count; ++i) {
    if (keys[i] == k) {
      return children[i];
    }
...
std::tuple<ArtTinyPtr, uint8_t> N4::getSecondChild(const uint8_t key) const {
  for (uint32_t i = 0; i < count; ++i) {
    if (keys[i] != key) {
...
std::pair<ArtTinyPtr, N4*> N4::Create(const uint8_t* prefix,
                                      uint32_t prefixLength,
                                      const TinyPtrHashes& h,
                                      ArtDerefTables& deref_tables) {
  auto [tinyPtr, ptr] = deref_tables.n4_deref_table.allocate(h, N4S);
  return {tinyPtr, new(ptr) N4(prefix, prefixLength)};
}
}